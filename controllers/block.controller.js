...
exports.create = async (req, res) => {
    ...
    if (req.body.isEmpty) {
        ...
        if (typeof (slot) != "number")
            return res.status(400).json({
                error: {
                    detail: "json: cannot unmarshal string into Go struct field BlockReq.slot of type int",
                },
                code: ""
            });
...
exports.GetBlockById = async (req, res) => {
    ...
    try {
      const result = await pool.query("SELECT * FROM block WHERE id = $1 and deleted_at is NULL", [
        blockId,
      ]);
      console.log(blockId);
